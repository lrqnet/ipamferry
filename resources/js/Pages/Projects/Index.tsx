...
export default function Index({ projects }: { projects: Project[] }) {
  ...
  return (
    <PageShell>
      <main className="mx-auto grid max-w-5xl gap-10 p-8 md:grid-cols-[1fr_2fr]">
        <Form
          action="/projects"
          method="post"
          className="rounded border border-slate-800 bg-slate-900 p-6"
        >
          <h1 className="text-xl font-bold">{t("projects.new")}</h1>
          ...
          <label className="mt-4 block text-sm">
            ...
            <select
              ...
              className="mt-1 w-full rounded bg-slate-800 p-2"
            >
              <option value="api">{t("projects.api")}</option>
              <option value="dump">{t("projects.dump")}</option>
            </select>
          </label>
          <label className="mt-4 block text-sm">
            {t("projects.locale")}
        ...
        <section>
          <h2 className="text-xl font-bold">{t("projects.existing")}</h2>
