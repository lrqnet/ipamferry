...
const sets = [
  ...
  ['docs/VALIDATION.md', 'docs/pt-BR/VALIDATION.md', 'docs/es/VALIDATION.md'],
...
for (const set of sets) {
  for (const file of set) {
    ...
    const languageLinks = links.filter((link) => /(?:README\.md|DOCUMENTATION\.md|CHANGELOG\.md|ARCHITECTURE\.md|DEVELOPMENT\.md|MAPPING-STUDIO\.md|RELEASE\.md|PLAN\.md|VALIDATION\.md|000[1-3]-.*\.md)$/.test(link));
